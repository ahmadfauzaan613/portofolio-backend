...
export const create = async (data: any) => {
  const query = `
    INSERT INTO portfolios (image_banner, short_desc, description, link, all_image, logo, category)
    VALUES ($1, $2, $3, $4, $5, $6, $7)
    RETURNING id, image_banner, short_desc, description, link, all_image, logo, category`
...
export const findAll = async (limit: number, offset: number, category?: string) => {
  let query = `
    SELECT id, image_banner, short_desc, description, link, all_image, logo, category
    FROM portfolios`
...
export const findById = async (id: number) => {
  const query = `SELECT id, image_banner, short_desc, description, link, all_image, logo, category FROM portfolios WHERE id = $1`
